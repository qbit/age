...
ReadLoop:
	...
	if len(out) > 1 {
		// Regrettably, we can't surface multiple recipient stanzas from the
		// current Wrap API. We'll need to add a WrapMultiple optional interface.
		return nil, fmt.Errorf("multiple recipient stanzas is currently unsupported")
	}
...
func (i *Identity) Unwrap(block *age.Stanza) (fileKey []byte, err error) {
	...
	// Phase 1: client sends the plugin the identity string and the stanza
	s := &format.Stanza{
		Type: "add-identity",
	...
	s = &format.Stanza{
		...
		Args: append([]string{"0", block.Type}, block.Args...),
		Body: block.Body,
	}
	if err := s.Marshal(stdin); err != nil {
		return nil, err
